...
export default function DateFilter({
  ...
  const getUrl = (p?: string, f?: string, t?: string) => {
  ...
  return (
    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
      <span className="text-mono text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', marginRight: '8px' }}>Date Filter:</span>
      <Link href={getUrl('today')} className={`btn btn-sm ${period === 'today' ? 'btn-primary' : ''}`}>Today</Link>
      <Link href={getUrl('week')} className={`btn btn-sm ${period === 'week' ? 'btn-primary' : ''}`}>This Week</Link>
      <Link href={getUrl('month')} className={`btn btn-sm ${period === 'month' ? 'btn-primary' : ''}`}>This Month</Link>
      <Link href={getUrl(undefined)} className={`btn btn-sm ${!period && !from && !to && !showCustom ? 'btn-primary' : ''}`}>All Time</Link>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {showCustom && (
          <div style={{ 
            ...
            <form method="GET" action="/settings/audit-log" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      {(period || from || to) && (
        <Link href={getUrl(undefined)} className="text-mono text-muted" style={{ fontSize: '11px', textDecoration: 'underline', marginLeft: 'auto' }}>Clear Date Filter</Link>
      )}
    </div>
  );
}
